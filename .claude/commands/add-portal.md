...
1. Ask whether the user wants the new portal added to their `/search-roles` search. If yes:
   - The portal CLI itself is already picked up automatically by `/search-roles` (it discovers `.agents/skills/*/SKILL.md`) — no further wiring is needed for CLI search/detail.
   - Optionally note the portal in `company-tracker/track_list.json` as an available resolution path for tracked companies whose ATS it covers.
...
- The generator is country-agnostic; add whatever portals your target companies actually use (Greenhouse, Ashby, Lever, or a market-specific board).
