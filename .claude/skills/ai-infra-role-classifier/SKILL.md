...
- **Adjacent** - Real infra work, but the role's center of gravity is *operating* or *supporting* the platform rather than *building* the inference/training/serving stack itself - e.g. general cloud-platform/deployment engineering, or a role split between infra and a genuinely separate discipline (like open-ended ML research). Also includes **AI agent infrastructure** - specifically infra that *serves or runs AI agents as a platform capability* (agent orchestration engines, agent execution runtime/sandboxing, tool-calling gateways offered to customers or as a product surface). This is narrower than "anything agent-related": internal tooling that merely *uses* AI agents to make a company's own engineers more productive is not agent infrastructure - it's an internal AI-agent product, and is excluded under pattern 3 the same as any other internal/product-scoped tooling.
...
3. **Product-facing engineer roles** - a posting scoped around customer-facing product features/UI rather than core infra, even when it says "Member of Technical Staff" or "Software Engineer." Signals: "product engineering, developer experience" framing as the role's center of gravity; frontend stack (React/TypeScript) as a primary skill; "Product (Backend)" or "Product Engineer" in the title. This also covers **internal-facing product tooling** built for a company's own employees rather than for customers or the AI/LLM infra stack itself - e.g. an internal AI-agent productivity platform that helps engineers code faster is a product (the "customer" is just internal), not infrastructure, even though it's AI-agent-flavored.
...
7. **SRE / Site Reliability Engineer / Reliability Engineering roles** (any variant, any company's naming convention) - excluded entirely, not Adjacent. These roles operate and keep reliable a platform that already exists, rather than building the inference/training/serving stack itself. (Confirmed exclusion, not a judgment call - decided explicitly, same status as pattern 6.)
...
| Member of Technical Staff - Reliability Engineering (Fireworks AI) | Excluded (SRE pattern) | Reliability/SRE work for an inference platform - operates it, doesn't build the inference stack itself (corrected 2026-08-26 - previously Adjacent, now excluded per pattern 7) |
...
| Site Reliability Engineer (Baseten) | Excluded (SRE pattern) | Day-2 ops for the ML infra platform - operates it, doesn't build the inference stack itself (corrected 2026-08-26 - previously Adjacent, now excluded per pattern 7) |
| Cloud Platform Engineer (Baseten) | Adjacent | General platform engineering (deployments, monitoring, incident response) - real infra work, not SRE-titled, but a different center of gravity from the inference stack itself |
| Software Engineer - AI Developer Productivity (Baseten) | Excluded (product-scoped) | Builds an internal AI-agent product (agent configs, context tooling, evals) for Baseten's own engineers - this *uses* AI agents to help engineers, it does not serve or run AI agents as infrastructure. Corrected 2026-08-26: previously miscategorized as "AI agent infrastructure" Adjacent; that category is narrower than "anything agent-related" (see the Adjacent definition above). |
