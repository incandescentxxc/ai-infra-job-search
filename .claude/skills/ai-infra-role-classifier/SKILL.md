...
- **Adjacent** - Real infra work, but the role's center of gravity is *operating* or *supporting* the platform rather than *building* the inference/training/serving stack itself - e.g. general cloud-platform/deployment engineering, a role split between infra and a genuinely separate discipline (like open-ended ML research), or **AI agent infrastructure** (agent orchestration platforms, agent tooling/runtime, internal agent dev-productivity platforms) - genuine infra work, but a different center of gravity from the core inference/training/serving stack.
...
| Software Engineer - AI Developer Productivity (Baseten) | Adjacent | AI agent infrastructure - internal agent-config/tooling platform for engineers, not the inference/training/serving stack itself |
